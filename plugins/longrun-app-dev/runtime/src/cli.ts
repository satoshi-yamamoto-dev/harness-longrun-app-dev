...
import { ensureProjectDependencies, findProjectPlaywrightCli } from "./evaluator/index.js";
import { Orchestrator, acquireRunLock, cleanRun, recoverRunLock } from "./orchestrator/index.js";
...
async function main(args: string[]): Promise<number> {
  ...
  if (command === "resume") {
    ...
    const result = await new Orchestrator(new DefaultSessionRunner(new ClaudeAgentSdkClient()), new WebProjectAdapter(root))
      .resume(root, request[0]!, { cliPath: await findProjectPlaywrightCli(root), browser: process.platform === "win32" ? "msedge" : "chromium" });
